...
export function AIDisclaimerModal({ isOpen, feature, onAccepted }: AIDisclaimerModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-950/70 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl border border-slate-200/80 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col">
        {/* Modal Header */}
        <div className="bg-gradient-to-r from-violet-600 to-indigo-600 p-6 text-white text-left">
        ...
        {/* Modal Footer */}
        <div className="p-5 sm:px-7 border-t border-slate-100 bg-slate-50/70 flex justify-end">
          <Button
            type="button"
            onClick={handleAccept}
            disabled={isPending}
            className="w-full sm:w-auto bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 text-white font-extrabold px-8 py-3 rounded-xl text-sm shadow-md shadow-indigo-200 transition-all cursor-pointer"
          >
            {isPending ? (
