...
import {
  ...
  Sparkles, X, Lock, Mail, User, Phone, Search, MoreVertical
} from "lucide-react";
...
export function PaymentsAdmin({
  ...
  const filteredSubs = subscriberList.filter(
    (s) =>
      ...
      s.subscription_status?.toLowerCase().includes(search.toLowerCase())
  );
    ...
    <div className="space-y-6">
      ...
      {activeTab === "subscriptions" && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-4 sm:p-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
            ...
            <table className="w-full text-left text-sm min-w-[850px]">
              <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-100 text-xs uppercase tracking-wider">
                <tr>
                  ...
                  <th className="px-5 py-4 whitespace-nowrap">Stripe Customer ID</th>
                      ...
                      <tr key={sub.id} className="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 bg-violet-100 text-violet-700 rounded-full flex items-center justify-center font-bold text-sm shrink-0 shadow-2xs">
                        ...
                        <td className="px-5 py-4 text-slate-600 text-xs sm:text-sm">
                          <span className="truncate block max-w-[220px]" title={sub.display_email}>
                            {sub.display_email}
                          </span>
                        </td>

                        {/* 3. Plan */}
                        <td className="px-5 py-4 font-semibold text-slate-900 whitespace-nowrap text-xs sm:text-sm">
                          {sub.plan_name}
                        </td>
                        ...
                        {/* 5. Stripe Customer ID */}
                        <td className="px-5 py-4 whitespace-nowrap">
                          <span className="font-mono text-xs text-slate-600 bg-slate-100 px-2.5 py-1 rounded-md border border-slate-200/60 inline-block" title={sub.stripe_customer_id || ""}>
